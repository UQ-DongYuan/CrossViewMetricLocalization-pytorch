...
class VIGOR(Dataset):
    ...
    def __len__(self):
        if self.train_test == 'test' or self.validation:  # test or validation
            return self.val_data_size
        elif self.train_test == 'train' and not self.validation:  # train
            return self.train_data_size

    def __getitem__(self, index):
        if self.train_test == 'train' and not self.validation:
            # load train ground image
            grd_img = Image.open(self.train_list[index])
            ...
            sat_img = Image.open(self.train_sat_list[self.train_label[index][pos_index]]).convert('RGB')
            sat_img = self.sat_transform(sat_img)

            [col_offset, row_offset] = self.train_delta[index, pos_index]  # delta = [delta_lat, delta_lon]
        ...
        if self.train_test == 'test' or self.validation:
            # load test/validation ground image
            grd_img = Image.open(self.val_list[index])
            grd_img = self.grd_transform(grd_img)

            # load test/validation satellite image
            pos_index = 0  # we use the positive (no semi-positive) satellite images during testing
            sat_img = Image.open(self.test_sat_list[self.val_label[index][pos_index]]).convert('RGB')
            sat_img = self.sat_transform(sat_img)

            [col_offset, row_offset] = self.val_delta[index, pos_index]  # delta = [delta_lat, delta_lon]
            row_offset_resized = (row_offset / 640 * self.sat_size[0]).astype(np.int32)
            col_offset_resized = (col_offset / 640 * self.sat_size[0]).astype(np.int32)
            # Gaussian GT
            x, y = np.meshgrid(
